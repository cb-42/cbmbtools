...
thresh <- 0.1
otu_good <- load_shared(shared = "data/shared", thresh)
...
phy_df <- phy_df %>%
  dplyr::select(Sample_name, colnames(phy_df[,-1])[colSums(phy_df[,-1]) > 0]) %>% # Retain only the phyla that were present in any of these samples
  inner_join(dplyr::select(otu_df, -contains("Otu")), by = "Sample_name") %>%
  dplyr::select(dplyr::select(otu_df, -contains("Otu")) %>% colnames(), everything())
```
...
phy_rank <- phy_df %>% 
  ...
  group_by(Phylum) %>%
  summarize(Mean_Perc = mean(Perc), .groups = "drop") %>%
...
After the data was processed using mothur(v.1.42.3), it was put through standardization methods for community ecology using the vegan package (version `r packageVersion("vegan")`) in R. To account for singletons and other extremely low abundance OTUs, OTUs which did not cross an abundance threshold of `r paste(thresh, "%")` in any sample were excluded. Of ___ total OTUs, ___ were denoted as Bacteria_unclassified and were also removed. Altogether there are `r dim(otu_good)[2]` remaining OTUs for further analysis. There are `r dim(otu_good)[1]` combined controls and specimens across all experiments.    
...
phy_agg <- phy_filt %>% 
  ...
  group_by(Phylum) %>%
  dplyr::summarize(Mean_Perc = mean(Percentage), SEM = sem(Percentage), .groups = "drop") %>%
